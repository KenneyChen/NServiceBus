    ...
    public class MessageDeserializerResolverTests
    {
        ...
        public void MultipleDeserializersWithSameContentTypeShouldThrowException()
        {
            ...
            Assert.That(() => new MessageDeserializerResolver(new FakeSerializer("xml"), new IMessageSerializer[]
            {
                deserializer1,
                deserializer2
            }), Throws.Exception.TypeOf<Exception>().And.Message.Contains($"Multiple deserializers are registered for content-type '{deserializer1.ContentType}'. Remove ambiguous deserializers for this content-type."));
        }
